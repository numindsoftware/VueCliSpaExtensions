...
namespace Numind.AspNetCore.SpaServices.Extensions
{
    internal class NodeScriptRunner
    {
        ...
        public NodeScriptRunner(string workingDirectory, string scriptName, string arguments, IDictionary<string, string> envVars, string packageManager)
        {
            ...
            var pkgExe = packageManager;
            var completeArguments = $"run {scriptName} -- {arguments ?? string.Empty}";
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                // On Windows, the package manager executable is a .cmd file, so it can't be executed
                // directly (except with UseShellExecute=true, but that's no good, because
                // it prevents capturing stdio). So we need to invoke it via "cmd /c".
                pkgExe = "cmd";
                completeArguments = $"/c {packageManager} {completeArguments}";
            }

            var processStartInfo = new ProcessStartInfo(pkgExe)
            {
